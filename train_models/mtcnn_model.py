...
def cls_ohem(cls_prob, label):
    ...
    label_filter_invalid = tf.nn.relu(label)
    loss = tf.losses.sparse_softmax_cross_entropy(
        tf.cast(label_filter_invalid, tf.int32),
        cls_prob,
    ...
    valid_num = tf.reduce_sum(valid_inds)

    keep_num = tf.cast(valid_num * num_keep_radio, dtype=tf.int32)
...
def bbox_ohem_smooth_L1_loss(bbox_pred, bbox_target, label):
    ...
    smooth_loss = tf.reduce_sum(tf.where(abs_error < threshold, loss_smaller,
                                         loss_larger),
                                axis=1)
    keep_num = tf.cast(tf.reduce_sum(valid_inds) * num_keep_radio,
                       dtype=tf.int32)
